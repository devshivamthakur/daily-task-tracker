...
import { isOverdue, isTaskCompletedForDate, todayISO } from "@/utils/tasks";

export function StatsCards({ tasks }: { tasks: Task[] }) {
  const active = tasks.filter((t) => !t.archived);
  const total = active.length;
  const completed = active.filter((t) => isTaskCompletedForDate(t, todayISO())).length;
  const pending = active.filter((t) => !isTaskCompletedForDate(t, todayISO())).length;
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
      {stats.map((s, i) => (
        ...
        >
          <div
            className={`absolute -right-6 -top-6 h-20 w-20 rounded-full bg-gradient-to-br ${s.color} opacity-15 blur-xl transition-opacity group-hover:opacity-30`}
          />
